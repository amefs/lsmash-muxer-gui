...
namespace MediaInfoLib
{
    ...
    public class MediaInfo : IDisposable
    {
        ...
        public void Close()
        {
            UnsafeNativeMethods.MediaInfo_Close(Handle); Handle = IntPtr.Zero;
        }
        ...
        {
            return (int)UnsafeNativeMethods.MediaInfo_State_Get(Handle);
        }

        public int Count_Get(StreamKind streamKind, int streamNumber = -1)
        {
            return (int)UnsafeNativeMethods.MediaInfo_Count_Get(Handle, (IntPtr)streamKind, (IntPtr)streamNumber);
        }
    ...
    public class MediaInfoList : IDisposable
    {
        ...
        {
            Handle = UnsafeNativeMethods.MediaInfoList_New();
        }

        public int Open(String fileName, InfoFileOptions options = InfoFileOptions.FileOption_Nothing)
        {
            return (int)UnsafeNativeMethods.MediaInfoList_Open(Handle, fileName, (IntPtr)options);
        }

        public void Close(int filePosition = -1)
        {
            UnsafeNativeMethods.MediaInfoList_Close(Handle, (IntPtr)filePosition); Handle = IntPtr.Zero;
        }

        public String Inform(int filePosition)
        {
            return Marshal.PtrToStringUni(UnsafeNativeMethods.MediaInfoList_Inform(Handle, (IntPtr)filePosition, (IntPtr)0));
        }

        public String Get(int filePosition, StreamKind streamKind, int streamNumber, String parameter, InfoKind kindOfInfo = InfoKind.Text, InfoKind kindOfSearch = InfoKind.Name)
        {
            return Marshal.PtrToStringUni(UnsafeNativeMethods.MediaInfoList_Get(Handle, (IntPtr)filePosition, (IntPtr)streamKind, (IntPtr)streamNumber, parameter, (IntPtr)kindOfInfo, (IntPtr)kindOfSearch));
        }

        public String Get(int filePosition, StreamKind streamKind, int streamNumber, int parameter, InfoKind kindOfInfo)
        {
            return Marshal.PtrToStringUni(UnsafeNativeMethods.MediaInfoList_GetI(Handle, (IntPtr)filePosition, (IntPtr)streamKind, (IntPtr)streamNumber, (IntPtr)parameter, (IntPtr)kindOfInfo));
        }

        public String Get(int filePosition, StreamKind streamKind, int streamNumber, int parameter)
        {
            return Get(filePosition, streamKind, streamNumber, parameter, InfoKind.Text);
        }

        public String Option(String optionName, String optionValue)
        {
            return Marshal.PtrToStringUni(UnsafeNativeMethods.MediaInfoList_Option(Handle, optionName, optionValue));
        }
        ...
        public int State_Get()
        {
            return (int)UnsafeNativeMethods.MediaInfoList_State_Get(Handle);
        }

        public int Count_Get(int filePosition, StreamKind streamKind, int streamNumber)
        {
            return (int)UnsafeNativeMethods.MediaInfoList_Count_Get(Handle, (IntPtr)filePosition, (IntPtr)streamKind, (IntPtr)streamNumber);
        }

        public int Count_Get(int filePosition, StreamKind streamKind)
        {
            return Count_Get(filePosition, streamKind, (int)StreamKind.None);
        }
